...
import { X, CircleCheck, Award, Lock } from "lucide-react";
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-[11px] text-white/60">
                        {t("earn.current", { n: r.stage?.current ?? 1 })}
                      </p>
                    </div>
                    <span className="rounded-full bg-brand px-2.5 py-1 text-[11px] font-medium">
                      {t("earn.currentPill")}
                    </span>
                  </div>
                  {r.stage && (
                    <div className="mt-2.5 flex gap-1.5 pl-12">
                      {Array.from({ length: r.stage.total }).map((_, i) => (
                        <span
                          key={i}
                          className={`h-1.5 flex-1 rounded-full ${i < r.stage!.current ? "bg-brand" : "bg-white/20"}`}
                        />
                      ))}
                    </div>
                  )}
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-[11px] text-slate-400">
                    {t("earn.levelState", {
                      n: r.levelIndex,
                      state: locked ? t("earn.locked") : t("earn.unlocked"),
                    })}
                  </p>
                </div>
