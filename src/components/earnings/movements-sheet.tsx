...
import { X, ChevronRight, UserCheck, Network, CircleCheck, Clock, Ban, Info } from "lucide-react";
                    ...
                    >
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="truncate text-[13px] font-medium text-slate-900">
                          {net ? t("earn.fromNetwork") : m.title}
                        </p>
...
function Row({ label, value, strong, muted }: { label: string; value: string; strong?: boolean; muted?: boolean }) {
  return (
    <div className="flex items-center justify-between border-b border-slate-50 py-2.5 last:border-0">
      <span className={`text-[13px] ${strong ? "font-medium text-slate-900" : muted ? "text-slate-400" : "text-slate-500"}`}>
        {label}
      </span>
      <span className={`text-[13px] tnum ${strong ? "font-medium text-slate-900" : muted ? "text-slate-400" : "text-slate-800"}`}>
        {value}
      </span>
    </div>
  );
